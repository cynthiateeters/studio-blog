...
<figure>
  <img src="https://res.cloudinary.com/cynthia-teeters/image/upload/f_auto,q_auto,w_480,c_limit/canvas/hap/hap-scientist" alt="HAP robot wearing glasses holding bubbling test tube with chemistry symbols" style="max-width: 480px; border-radius: 5%;" loading="lazy" />
  <figcaption>HAP takes security seriously — lab coat and all.</figcaption>
</figure>
...
<DataList items={[
  { term: "Displaying user input without sanitizing", definition: "Attackers run code in your users' browsers (XSS)" },
  { term: "Putting API keys in source code", definition: "Anyone with repo access can steal credentials" },
  { term: "Ignoring outdated dependencies", definition: "Known exploits that automated tools find in seconds" }
]} />
